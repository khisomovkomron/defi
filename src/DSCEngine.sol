...
contract DSCEngine is ReentrancyGuard {
    ...
    event CollateralRedeemed(
        address indexed redeemedFrom, address indexed tokredeemedTo, address indexed token, uint256 amount
    );
     ...
     * @param user The user who has broken the health factor. Thei _helthFactor should be
     * below in MIN_HEALTH_FACTOR
    ...
    function liqudate(address collateral, address user, uint256 debtToCover)
        ...
        nonReentrant
    {
        uint256 startingUserHealthFactor = _healthfFactor(user);
        ...
        uint256 totalColalteralToRedeem = tokenAmountFromDebtCovered + bonusCollateral;
        _redeemCollateral(user, msg.sender, collateral, totalColalteralToRedeem);
        _burnDsc(debtToCover, user, msg.sender);
        
        uint256 endingUserHealthFactor = _healthfFactor(user);
